...
shinyServer(function(input, output, session) {
  ...
  # Updates the search box if only stem fields are selected
  ...
  output$employment_chart <- renderPlotly({
    ...
    bar_chart <- plot_ly(
      ...
      type = "bar",
      marker = list(color = c(
        "rgba(120,194,174,1)", "rgba(120,194,174,1)",
  ...
  output$gender_chart <- renderPlotly({
    ...
    if (input$checkbox == FALSE) {
      ...
      pie_chart <- plot_ly(gender_grads1,
        labels = (colnames(gender_grads1[1:2])), values = g_data, type = "pie",
      ...
      colors <- c("rgb(10, 147, 80)", "rgb(115, 226, 172)")
      pie_chart <- plot_ly(gender_stem_grads,
        labels = (colnames(gender_stem_grads[1:2])), values = g_data, type = "pie",
        insidetextfont = list(color = "#FFFFFF"),
        marker = list(
          colors = colors,
          line = list(color = "#FFFFFF", width = 1),
